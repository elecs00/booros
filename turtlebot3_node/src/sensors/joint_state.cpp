...
static std::array<int32_t, robotis::turtlebot3::sensors::MOTOR_NUM> last_diff_position,
  last_position;
...
: Sensors(nh, frame_id)
{
  ...
  if (name_space_ != "") {
    ...
    wheel_rear_right_joint_ = name_space_ + "/RB_wheel_joint";
    wheel_rear_left_joint_ = name_space_ + "/LB_wheel_joint";
    wheel_front_right_joint_ = name_space_ + "/RF_wheel_joint";
    wheel_front_left_joint_ = name_space_ + "/LF_wheel_joint";
  }
  ...
  std::shared_ptr<DynamixelSDKWrapper> & dxl_sdk_wrapper)
{
  ...
  std::array<int32_t, robotis::turtlebot3::sensors::MOTOR_NUM> position =
  {dxl_sdk_wrapper->get_data_from_device<int32_t>(
      extern_control_table.present_position_left.addr,
      extern_control_table.present_position_left.length),
    dxl_sdk_wrapper->get_data_from_device<int32_t>(
  ...
  std::array<int32_t, robotis::turtlebot3::sensors::MOTOR_NUM> velocity =
  {dxl_sdk_wrapper->get_data_from_device<int32_t>(
  ...
  msg->name.push_back(wheel_rear_left_joint_);   // LB_wheel_joint
  msg->name.push_back(wheel_rear_right_joint_);  // RB_wheel_joint
  msg->name.push_back(wheel_front_left_joint_);  // LF_wheel_joint
  msg->name.push_back(wheel_front_right_joint_); // RF_wheel_joint

  msg->position.push_back(TICK_TO_RAD * last_diff_position[0]);
  msg->position.push_back(TICK_TO_RAD * last_diff_position[1]);
  msg->position.push_back(TICK_TO_RAD * last_diff_position[0]);  // 앞 왼쪽 바퀴 (데이터 복제)
  msg->position.push_back(TICK_TO_RAD * last_diff_position[1]);  // 앞 오른쪽 바퀴 (데이터 복제)

  msg->velocity.push_back(RPM_TO_MS * velocity[0]);
  msg->velocity.push_back(RPM_TO_MS * velocity[1]);
  msg->velocity.push_back(RPM_TO_MS * velocity[0]);  // 앞 왼쪽 바퀴 (데이터 복제)
  msg->velocity.push_back(RPM_TO_MS * velocity[1]);  // 앞 오른쪽 바퀴 (데이터 복제)
