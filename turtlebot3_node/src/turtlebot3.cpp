  ...
  cmd_vel_sub_ = std::make_unique<TwistSubscriber>(
    ...
    std::function<void(const geometry_msgs::msg::Twist::SharedPtr)>(
      [this](const geometry_msgs::msg::Twist::SharedPtr msg) -> void
      {
        ...
        // 4륜 구동 계산
        float wheel_separation = wheels_.separation;
        float wheel_radius = wheels_.radius;
        
        // 좌우 바퀴 속도 계산 (차동 구동)
        float left_velocity = linear_x - (angular_z * wheel_separation / 2.0);
        float right_velocity = linear_x + (angular_z * wheel_separation / 2.0);
        
        // 속도 제한
        float max_velocity = 0.5; // m/s
        left_velocity = std::max(-max_velocity, std::min(max_velocity, left_velocity));
        right_velocity = std::max(-max_velocity, std::min(max_velocity, right_velocity));
        ...
        // 4륜 구동: 앞/뒤 바퀴가 같은 속도로 움직임
        data.dword[0] = static_cast<int32_t>(left_velocity * 100);   // ID 1 (Left-Front)
        data.dword[1] = static_cast<int32_t>(right_velocity * 100);  // ID 2 (Right-Front)  
        data.dword[2] = static_cast<int32_t>(left_velocity * 100);   // ID 3 (Left-Rear)
        data.dword[3] = static_cast<int32_t>(right_velocity * 100);  // ID 4 (Right-Rear)
        data.dword[4] = 0;  // 사용하지 않음
        data.dword[5] = 0;  // 사용하지 않음
        ...
        RCLCPP_DEBUG(
          this->get_logger(),
          "4WD - lin_vel: %f ang_vel: %f left: %f right: %f msg: %s", 
          linear_x, angular_z, left_velocity, right_velocity, sdk_msg.c_str());
      }
    ),
